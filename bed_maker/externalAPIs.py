...
import shutil
import re
from bed_maker.models import ClinvarCoverage_grch37, ClinvarCoverage_grch38
import ijson
import json
import wget
...
def download_Clinvar_data():
    ...
    os.mkdir(cur_path + "/Clinvar/grch_38")
    os.mkdir(cur_path + "/Clinvar/grch_37")
    print('Downloading grch38 Clinvar Data ...')
    wget.download(
        "https://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh38/clinvar.vcf.gz",
        out=cur_path + "/Clinvar/grch_38",
    )
    wget.download(
    ...
    def get_variant_by_gene(genome):
        if genome == 'grch37':
            read_path = cur_path + "/Clinvar/grch_37/clinvar.vcf.gz"
            save_path = cur_path + "/Clinvar/grch_37/grch37_clinvar_variant_per_gene.pkl"
        elif genome == 'grch38':
            read_path = cur_path + "/Clinvar/grch_38/clinvar.vcf.gz"
            save_path = cur_path + "/Clinvar/grch_38/grch38_clinvar_variant_per_gene.pkl"
        vcfReader = vcf.Reader(
            filename=read_path, compressed=True, encoding="utf-8"
        )
        gene_list = []
        ...
        clinvar_variant_per_gene = Counter(gene_list)
        with open(
            save_path, "wb"
            ) as outputfile:
                pickle.dump(clinvar_variant_per_gene, outputfile)
    print('Calculating grch37 variants per gene')
    get_variant_by_gene('grch37')
    print('Calculating grch38 variants per gene')
    get_variant_by_gene('grch38')


def download_Clinvar_grch37_data():
    if os.path.exists(cur_path + "/Clinvar_grch37"):
        shutil.rmtree(cur_path + "/Clinvar_grch37")
    os.mkdir(cur_path + "/Clinvar_grch37")
    wget.download(
        "https://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh37/clinvar.vcf.gz",
        out=cur_path + "/Clinvar_grch37",
    )
    wget.download(
        "https://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh37/clinvar.vcf.gz.tbi",
        out=cur_path + "/Clinvar_grch37",
    ...
    wget.download(
        "http://ftp.ensembl.org/pub/release-105/tsv/homo_sapiens/Homo_sapiens.GRCh38.105.canonical.tsv.gz",
        out=cur_path + "/Clinvar_grch37",
    )


def create_grch37_json():
    '''
    This function creates a json file for Grch37 gene transcrpts and exon information. 
    The output is to be used for calculating Clinvar coverage for the Grch37 genome. 
    If the csv file is not aviaalbe it can be created from http://grch37.ensembl.org/biomart
    
    Select the follwing culumns to create the csv file: 
        - Gene stable ID 
        - Gene name
    ...
    Save the file as "grch37_gene_annotations.csv" compress it and place in the "Ensembl_grch37" folder
    '''
    data = pd.read_csv(cur_path + "/Ensembl_grch37/grch37_gene_annotations.csv.gz", sep=",", compression='gzip')
    new_data = data.groupby(data['Gene stable ID'])
    gene_dict = {}
    for gene_id, gene_data in new_data:
        try:
            name = gene_data['Gene name'].unique()[0]
        except:
            name = ''
        trans_grouped = gene_data.groupby('Transcript stable ID')
        trans = []
        for trans_id, trans_group in trans_grouped:
            exons=[]
            for i in trans_group.index:
                exonID = trans_group["Exon stable ID"][i]
                try:
                    exon_start = str(int(trans_group["Exon region start (bp)"][i]))
                    exon_end = str(int(trans_group["Exon region end (bp)"][i]))
                    chrom = str(trans_group["Chromosome/scaffold name"][i])
                except:
                    exon_start = ''
                    exon_end = ''
                    chrom = ''
                exon_dict = { 'id': exonID,"seq_region_name": chrom, 'start' : exon_start, 'end': exon_end }
                exons.append(exon_dict)
            trans.append({'id' : trans_id, 'exons': exons})
        gene_dict[gene_id] = {'name' : name, 'transcripts' : trans}

    with open(cur_path + "/Ensembl_grch37/grch37_filtered_data.json", "w") as fp: 
        json.dump(gene_dict, fp, indent=4)
...
def download_ensembl_annotations():
    ...
    """
    if os.path.exists(cur_path + "/Ensembl_grch38"):
        shutil.rmtree(cur_path + "/Ensembl_grch38")
    os.mkdir(cur_path + "/Ensembl_grch38")

    wget.download(
        "http://ftp.ensembl.org/pub/release-105/tsv/homo_sapiens/Homo_sapiens.GRCh38.105.canonical.tsv.gz",
        out=cur_path + "/Ensembl_grch38",
    )
    wget.download(
        "http://ftp.ensembl.org/pub/release-105/json/homo_sapiens/homo_sapiens.json",
        out=cur_path + "/Ensembl_grch38",
    )
   
    my_dict = {}
    with open(cur_path + "/Ensembl_grch38/homo_sapiens.json", "r") as f:
        ...
        for obj in ijson.items(parse_events, "genes.item"):
            ...
            my_dict[obj["id"]] = {"name": name, "transcripts": trans}

    with open(cur_path + "/Ensembl_grch38/filtered_data.json", "w") as fp:
...
def get_MANE_list():
    ...
    if os.path.isfile(cur_path + "/external_data/MANElist.csv"):
        MANE_list_df = pd.read_csv(
            cur_path + "/external_data/MANElist.csv",
        )
    else:
        ...
        MANE_list_df.to_csv(
            cur_path + "/external_data/MANElist.csv", index=False, encoding="utf-8"
        )
    return MANE_list_df
...
def count_clinvar_variants_per_interval(
    variant_type=["All"], clinical_significance=["All"]
):
    """
    ...
    vcfReader = vcf.Reader(
        filename=cur_path + "/Clinvar/grch37/clinvar.vcf.gz", compressed=True, encoding="utf-8"
    )
...
def get_clinvar_variant_summary(variant_type=["All"], clinical_significance=["All"]):
    ...
    if os.path.isfile(cur_path + "/Clinvar/clinvar_variant_per_gene.pkl"):
        with open(
            cur_path + "/Clinvar/clinvar_variant_per_gene.pkl", "rb"
        ) as inputfile:
            clinvar_variant_per_gene = pickle.load(inputfile)   
    else:
        # If no cache read in ClinVar vcf
        #vcfReader = vcf.Reader(
         #   filename=cur_path + "/Clinvar/clinvar.vcf.gz",
         #   compressed=True,
         #   encoding="utf-8",
       # )
        clinvar_variant_per_gene = count_clinvar_variants_per_interval(
           variant_type, clinical_significance
        )
        # Save summary ClinVar data for faster loading next time
        with open(
            cur_path + "/Clinvar/clinvar_variant_per_gene.pkl", "wb"
        ) as outputfile:
            pickle.dump(clinvar_variant_per_gene, outputfile)
...
def get_HGMD_data():
    ...
    if os.path.isfile(cur_path + "/external_data/RefSeqHGMDtranscripts.pkl"):
        with open(
            cur_path + "/external_data/RefSeqHGMDtranscripts.pkl", "rb"
        ) as inputfile:
        ...
        # Save summary ClinVar data for faster loading next time
        with open(
            cur_path + "/external_data/RefSeqHGMDtranscripts.pkl", "wb"
        ) as outputfile:
            pickle.dump(HGMD_list, outputfile)
    return HGMD_list


def get_variants_by_interval(
    intervals, genome, variant_type=["All"], clinical_significance=["All"]
):
    """
    ...
    """
    if genome == 'grch37':
        path = cur_path + "/Clinvar/grch_37/clinvar.vcf.gz"
    elif genome == 'grch38':
        path = cur_path + "/Clinvar/grch_38/clinvar.vcf.gz"
    vcfReader = vcf.Reader(
        filename=path, compressed=True, encoding="utf-8"
    )
    result = set()
        ...
        try:
            for record in vcfReader.fetch(chrom, int(start), int(end)):
                if variant_type == ["All"] and clinical_significance == ["All"]:
                    ...
                    if (
                        record.INFO["CLNVC"] in variant_type
                        and record.INFO["CLNSIG"] in clinical_significance
                    ):
                        result.add(record.ID)
        except:
            print('region not found in the Clinvar VCF: {}:{}-{}'.format(chrom, start,end))
    return list(result)


def calculate_clinvar_coverage_for_all_ensembl_transcripts(genome):
    """
    Pre-calculate the clinvar coverage for all Ensembl transcripts
    """
   
    if genome == 'grch38':
        ClinvarCoverage_grch38.objects.all().delete()
        with open(
            cur_path + "/Clinvar/grch_38/grch38_clinvar_variant_per_gene.pkl", "rb"
        ) as inputfile:
            variant_count_per_gene = pickle.load(inputfile)   
        #variant_count_per_gene = get_clinvar_variant_summary(["single_nucleotide_variant"])
        ensembl_Genes = pd.read_csv(
            cur_path + "/Ensembl_grch38/Homo_sapiens.GRCh38.105.canonical.tsv.gz",
            sep="\t",
            compression="gzip",
            engine="python",
        )
        jdata = json.load(open(cur_path + "/Ensembl_grch38/filtered_data.json"))
        pattern = re.compile(r"^(\S+)\.")
    elif genome == 'grch37':
        ClinvarCoverage_grch37.objects.all().delete()
        with open(
            cur_path + "/Clinvar/grch_37/grch37_clinvar_variant_per_gene.pkl", "rb"
        ) as inputfile:
            variant_count_per_gene = pickle.load(inputfile)   
        #variant_count_per_gene = get_clinvar_variant_summary(["single_nucleotide_variant"])
        ensembl_Genes = pd.read_csv(
            cur_path + "/Ensembl_grch37/Grch37_ensembl_genes.csv.gz",
            sep=",",
            compression="gzip",
            engine="python",
        )
        ensembl_Genes = ensembl_Genes.rename(columns={'Gene stable ID' : 'gene_stable_id'})
        jdata = json.load(open(cur_path + "/Ensembl_grch37/grch37_filtered_data.json"))
        pattern = re.compile(r"^(\S+)")
    ...
    for i in range(0, len(ensembl_Genes)):
        ...
        gene_variants_count = (variant_count_per_gene[gene_data["name"]] 
            if variant_count_per_gene[gene_data["name"]]>0
            else None
        )
        for transcript in gene_data["transcripts"]:
            transcript_intervals = get_transcript_intervals(transcript)
            variants = (
                get_variants_by_interval(
            ...
            clinvar_coverage_fraction = (
                round((len(variants) / gene_variants_count), 10)
                if gene_variants_count and len(variants)>0
                else 'NoInfo',
            )
            clinvar_variants = (len(variants) if variants else 'NoInfo',)
            print('Transcript: {}; Variant count per gene: {}; Clinvar variants: {}; Coverage: {}; Gene: {};  '.format(transcript["id"], gene_variants_count, clinvar_variants[0], clinvar_coverage_fraction[0], gene_data["name"]))
            if genome == 'grch37':
                ClinvarData = ClinvarCoverage_grch37.objects.get_or_create(
...
def get_panel_app_list():
    ...
    if os.path.isfile(cur_path + "/external_data/GEL_panel_app.csv"):
        GEL_panel_app_df = pd.read_csv(
            cur_path + "/external_data/GEL_panel_app.csv",
        )
    else:
        # if no cache download MANE list from TARK API
        ...
        GEL_panel_app_df.to_csv(
            cur_path + "/external_data/GEL_panel_app.csv", index=False, encoding="utf-8"
        )
...
def get_panel_app_genes(panel_id):
    print(
        f"https://panelapp.genomicsengland.co.uk/WebServices/get_panel/{panel_id}/?format=json"
    )
